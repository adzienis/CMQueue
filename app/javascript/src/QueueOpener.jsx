...
export default props => {
    ...
    return (
        <button
            className={`btn btn-primary queue-button w-100 btn-${openStatus ? 'success' : (((openLoading && !queueLoading) ? 'secondary' : 'danger'))}`}
            onClick={e => {
                try {
                    mutateAsync()
                } catch (e) {
            ...
            {(openLoading) ?
                ...
                </div> : (<DelayedSpinner loading={queueLoading || openLoading}>
                    <span>
