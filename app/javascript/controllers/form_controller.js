...
export default class extends Controller {
    ...
    connect() {
        const inputElements = [...this.element.querySelectorAll("input,textarea,select")].filter(v => v.name !== "authenticity_token")
        ...
        inputElements.forEach(v => {
            ...
            if (v.classList.contains("is-invalid")) {
                ...
                this.originalValues.set(v, v instanceof HTMLSelectElement ? getSelectValues(v) : v.value);
            }

            if (v instanceof HTMLSelectElement) {
                v.onchange = e => {
                    if (inputEqualToValue(this.originalValues.get(e), e)) this.disabled = true;
                    else {
                        this.submitTarget.disabled = false
                        return;
                    }
