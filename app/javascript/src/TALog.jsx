...
const Component = (props) => {
    ...
    const {data: question_states, isFetching, isLoading} = useQuery(['courses', parseInt(courseId, 10), 'question_states', 'ta_feed', '?', `date=${date.toISOString()}`], {
        placeholderData: []
    })
    ...
    const ticks = useMemo(() => {
        if (question_states.length > 0) {
            const mapped = question_states.map((v) => new Date(v.created_at).getTime());
            ...
            const numTicks =
                (endTickDate.getTime() - startTickDate.getTime()) /
                1000 /
                60 /
                15;
            const ticksMapped = new Array(Math.ceil(numTicks) + 1)
                .fill(null)
                .map(
                    (_, ind) => startTickDate.getTime() + 1000 * 60 * 15 * ind,
                );
                ...
                <div>
                    <form>
                        <div>
                            ...
                            <input type="date" className="form-control w-auto" onChange={e => setDate(new Date(e.target.value))} value={date.toISOString().slice(0,10)}/>
                        </div>
                    </form>
                </div>
            </div>

            {isLoading || isFetching ? <div  style={{ height: '470px'}} className="d-flex justify-content-center align-items-center w-100 bg-white">

                <div className="spinner-border"role="status">
                    <span className="visually-hidden">Loading...</span>
                </div>
            </div> : (question_states?.length > 0 ? (
                <div className="">
