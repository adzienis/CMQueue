class Analytics::Queries::TimePerTa
  ...
  def call
    ...
    Enrollment.active.with_course_roles(:instructor, :ta).joins(:user, :question_states, question_states: :question)
      ...
      .select(
        "enrollments.id as enrollment_id, users.given_name || ' ' || users.family_name as full_name, (#{Question.where(
          id: QuestionState
                ...
                .select("question_id")
        ).with_today.select("count(*)").to_sql}) questions_total, extract(epoch from avg((#{QuestionState
                                ...
                                - (#{QuestionState.where(id: query.call("resolving").select("max(question_states.id)"))
                                                  .select(:created_at).to_sql}))) avg_seconds"
      ).order("questions_total DESC")
  end
