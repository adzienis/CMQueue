  ...
  const binnedData = useMemo(() => {
    if (taStatesData?.length > 0) {
      const mapped = taStatesData
        .map((v) => [new Date(v.start).getTime(), new Date(v.end).getTime()])
        .flat();
      ...
      const numTicks =
        (endDataDate.getTime() - startDataDate.getTime()) / 1000 / 60 / 15;
      const ticksMapped = new Array(Math.ceil(numTicks) + 2)
        ...
        .map((_, ind) => {
          ...
          const waitTime = taStatesData
            ...
            .map((v) =>
              v.end > timeIdx ? timeIdx - v.start : v.end - v.start
            );

          const clampedWait =
            waitTime.length > 0
              ? waitTime.reduce((acc, cur) => acc + cur) / waitTime.length
              : 0;
